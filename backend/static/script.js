...
function positionDialMarkers() {
  ...
  document.querySelectorAll('.dmk').forEach(mk => {
    ...
    // Convert from 0-180 servo angle to visual position on dial
    // Map 0° → left side, 90° → top, 180° → right side
...
async function pollStatus() {
  try {
    ...
    document.getElementById('sysDot').style.background = '#ff3c3c';
    document.getElementById('sysText').textContent = 'OFFLINE';
  }
}
...
function syncServoUI(data) {
  if (!simRunning) {
    // Sync servo 1 arm to actual backend angle
...
function setDialArm(angleDeg) {
  // Map 0-180° servo angle to visual rotation
  // Servo 0° = pointing left, 90° = pointing up, 180° = pointing right
